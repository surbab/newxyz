...
<div class="saleRfqs form">
<?php echo $this->Form->create('SaleRfq'); ?>
	<fieldset>
		<legend><?php echo __('Edit SaleRfq'); ?></legend>
	<?php
		echo $this->Form->hidden('location_id', array('default'=> 1));
		echo $this->Form->input('salerfq_ref',array('label' =>'Salerfq Reference No'));
		echo $this->Form->input('salerfq_no',array('label' =>'Salerfq No'));
		echo $this->Form->input('srn_date',array('label' =>'Salerfq Date'));
	...
	<div id="sr_details">
		...
		<div class='wrapper'>
			<?php
				if($poDets) {
					foreach($poDets as $saleRfqs) {
						echo $this->element('poDetails', array('saleRfqs' => $saleRfqs));
					}
				} else {
	...
	<?php	
		...
		echo $this->Form->input('po_desc',array('label' =>'Salerfq Description'));
...
<div class="actions">
	...
	<ul>
		...
		<li><?php echo $this->Form->postLink(__('Delete'), array('action' => 'delete', $this->Form->value('SaleRfq.id')), null, __('Are you sure you want to delete # %s?', $this->Form->value('SaleRfq.id'))); ?></li>
		...
		<li><?php echo $this->Html->link(__('List SaleRfq Dets'), array('controller' => 'saleRfq_dets', 'action' => 'index')); ?> </li>
		<li><?php echo $this->Html->link(__('New SaleRfq Det'), array('controller' => 'saleRfq_dets', 'action' => 'add')); ?> </li>
	</ul>
</div>
<script>
...
function calculateSum() {
        ...
        $("#SaleRfqTotValue").val(sum.toFixed(2));
}
...
function addNewRow(Obj) {
	...
	//$('#sr_details .wrapper div.row').first().clone().appendTo("#sr_details .wrapper");
}
